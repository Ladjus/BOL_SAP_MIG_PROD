...
SELECT
  ...
  CASE WHEN ar_2000.q_saps4_sortiment IS NOT NULL THEN CAST(GETDATE() AS date) ELSE CAST(NULL AS date) END
  ...
  CASE ar_op.anskaffningssatt
    ...
    WHEN 2 THEN  -- Köp mot behov (2) ==> tag Förpackningsstorlek förs ansk ar_op.q_artfsgforp decimal(14,3).
      CASE
        ...
        WHEN ar_op.q_artfsgforp > 0.0 THEN CAST(ROUND(ar_op.q_artfsgforp, 3) AS varchar(16))  -- decimal(14,3) as varchar(16).
        ELSE CAST('' AS varchar(16))
        -- Mix integer, decimal and '' in varchar column
      END
    ELSE CAST('' AS varchar(16))
