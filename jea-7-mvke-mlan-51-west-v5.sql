...
v.5: Add distribution channel 50 via UNION ALL SELECT.
     ...
     !!! Fork from jea-5-mvke-mlan-21-w-v5.sql to jea-7-mvke-mlan-51-w-v5.sql for articles with extra4 = 51 !!!
     Include Jeeves assortment values DA, DB, DC for duplicates of ECC articles.
     Column MVKE_PRODUCT gets Pacudo-nr to match POI ECC (not: artbeskrspec, "Artikelnr").
     Column MLAN_PRODUCT gets Pacudo-nr to match POI ECC (not: artbeskrspec, "Artikelnr").
*/
...
WITH cte_artnr_west AS (
  ...
  WHERE
    ...
    AND ar_2000.extra4 IN (51)  -- Change v.5.
/* Remove v.3.
...
*/
    AND (ar_2000.q_saps4_sortiment IN ('A', 'B', 'DA', 'DB')  -- A/B-sortiment: alla artiklar. Change v.3. Change v.5.
         OR (ar_2000.q_saps4_sortiment IN ('C', 'DC') AND ar_9100.artnr IS NOT NULL))  -- C-sortiment: endast artiklar som finns i Väst 9100. Change v.3. Change v.5.
)
-- Distribution channel 10
SELECT
  ar_2000.artnr AS AR_ArtNr,  -- Jeeves "Artikel ID"
--  ar_2000.artbeskrspec AS MVKE_PRODUCT,  -- SAP Product. Jeeves "Artikelnr". Remove v.4.
  ar_2000.q_artnr_pacudo AS MVKE_PRODUCT,  -- SAP Product. Jeeves "Pacudo ArtNr". Add v.4.
...
--  COALESCE(ar_2000.q_saps4_sortiment, '') AS MVKE_VMSTA,  -- SAP sales status per assortment. Change v.1. Remove v.5.
  CASE  -- Add v.5.
    WHEN ar_2000.q_saps4_sortiment IN ('A', 'B', 'C', 'DA', 'DB', 'DC')  -- A/B/C. Change v.5.
...
--  ar_2000.artbeskrspec AS MLAN_PRODUCT,  -- SAP Product. Change v.2 _2000. Remove v.4.
  ar_2000.q_artnr_pacudo AS MLAN_PRODUCT,  -- SAP Product. Jeeves "Pacudo ArtNr". Add v.4.
  'WEST' AS MLAN_RUN_ID,  -- Add v.2.
  'SE' AS MLAN_ALAND,
  'TTX1' AS MLAN_TATYP1,
