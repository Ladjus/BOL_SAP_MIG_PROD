...
v.5: CTE cte_artnr_west: Select A/B-assortment plus C for PacsOn Väst.
     !!! Fork from jea-3-marc-mbew-11-12-w-v5.sql to jea-5-marc-mbew-21-w-v5.sql for articles with extra4 = 21 !!!
v.6: Availability check MARC_MTVFP is Z2 for Jeeves-companies (was SP).
     !!! Fork from jea-5-marc-mbew-21-w-v6.sql to jea-7-marc-mbew-51-w-v6.sql for articles with extra4 = 51 !!!
     Include Jeeves assortment values DA, DB, DC for duplicates of ECC articles.
     Column MARC_PRODUCT gets Pacudo-nr to match POI ECC (not: artbeskrspec, "Artikelnr").
*/
...
WITH cte_artnr_west AS (
  ...
  WHERE
    ...
    AND ar_2000.extra4 IN (51)  -- Change v.6.
/* Remove v.5.
...
*/
    AND (ar_2000.q_saps4_sortiment IN ('A', 'B', 'DA', 'DB')  -- A/B-sortiment: alla artiklar. Change v.5. Change v.6.
         OR (ar_2000.q_saps4_sortiment IN ('C', 'DC') AND ar_9100.artnr IS NOT NULL))  -- C-sortiment: endast artiklar som finns i Väst 9100. Change v.5. Change v.6.
)
-- PacsOn Väst 9100, Falköping lager 5000.
SELECT
  ar_2000.artnr AS AR_ArtNr,  -- Jeeves "Artikel ID"
--  ar_2000.artbeskrspec AS MARC_PRODUCT,  -- SAP Product. Jeeves "Artikelnr". Remove v.4.
  ar_2000.q_artnr_pacudo AS MARC_PRODUCT,  -- SAP Product. Jeeves "Pacudo ArtNr". Add v.4.
