...
v.2. Align output for ECC_MATNR which was changed to character-type in PROD.
     Comment query (2).
     Extend query (3) with (3.d).
    !!! Fork from jea-2-class-art-ref-11-12-v2.sql to jea-4-class-art-ref-21-v2.sql for articles with extra4 = 21 !!!
*/
...
SELECT
  ...
  ar.artbeskrspec AS KSSK_PRODUCT,  -- SAP Product. Jeeves "Artikelnr"
  '1' AS KSSK_STATU  -- Classification status
FROM ar
WHERE
  ar.foretagkod in (2000)  -- Mall
  AND ar.extra4 IN (21)  -- Change v.2.
ORDER BY 4;
...
SELECT
  ...
  ar.artbeskrspec AS AUSP_CHAR_PRODUCT,  -- SAP Product. Jeeves "Artikelnr"
...
UNION ALL  -- Disjoint datasets, different AUSP_NUM_CHARACT, no duplicates. Add v.2.

-- Add v.2.
-- (3.d) Characteristic ECC_MATNR "ECC Material Nr [MARA.MATNR]"
SELECT
  ar.artnr AS AR_ArtNr,  -- Jeeves "Artikel ID"
  '#001' AS AUSP_CHAR_KLART,  -- Class type. # to prevent excel strip leading zero
  'ART_REF' AS AUSP_CHAR_CLASSNUM,  -- Class name
  ar.artbeskrspec AS AUSP_CHAR_PRODUCT,  -- SAP Product. Jeeves "Artikelnr"
  'ECC_MATNR' AS AUSP_CHAR_CHARACT,  -- Characteristic name
  '_001' AS AUSP_CHAR_POSNR,  -- Item number. _ to prevent excel strip leading zero, remove _ in load-xml.
  ar.q_artnr_pacudo AS AUSP_CHAR_ATFLV  -- Characteristic Value Neutral Long
FROM ar
WHERE
